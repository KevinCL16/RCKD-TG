...
from utils.dataset import TokenizedDataset, TokenizedEvalDataset
...
def main() -> None:
    ...
    if not args.arg_paths:
        ...
        os.makedirs(cache_root, exist_ok=True)
        meta_tuning_data = {}
        # TODO: Commented by Zhiyu: At this moment, if using rationale to do multi-task learning, then do extra question,
        #  shuffled table and consistency training at the same time
        if args.model.name == 'unified.rationale_mtl':
            for task, arg_path in args.arg_paths:
                ...
                task_seq2seq_dataset_split: tuple = utils.tool.get_constructor(task_args.seq2seq.constructor)(
                    task_args). \
                    to_seq2seq(task_raw_datasets_split, cache_root, training_args.use_extra_question,
                               training_args.use_shuffled_table)
            ...
            for task, arg_path in args.arg_paths:
                ...
                use different dataset loader in ./tasks to load original or personalized dataset
                
                """
                if training_args.use_extra_question and training_args.use_shuffled_table is True:
                    raise AssertionError("only one customized data format can be used. Combination of both not supported yet")

                elif training_args.use_extra_question is True:
                    task_raw_datasets_split: datasets.DatasetDict = datasets.load_dataset(
                        path=task_args.dataset.loader_path_extra_question,
                        cache_dir=task_args.dataset.data_store_path)

                elif training_args.use_shuffled_table is True:
                    task_raw_datasets_split: datasets.DatasetDict = datasets.load_dataset(
                        path=task_args.dataset.loader_path_shuffled_table,
                        cache_dir=task_args.dataset.data_store_path)
                ...
                task_seq2seq_dataset_split: tuple = utils.tool.get_constructor(task_args.seq2seq.constructor)(
                    task_args). \
                    to_seq2seq(task_raw_datasets_split, cache_root, training_args.use_extra_question, training_args.use_shuffled_table)
    ...
    eval_dataset = TokenizedEvalDataset(args, training_args, model_tokenizer,
                                        seq2seq_eval_dataset) if seq2seq_eval_dataset else None
    test_dataset = TokenizedEvalDataset(args, training_args, model_tokenizer,
