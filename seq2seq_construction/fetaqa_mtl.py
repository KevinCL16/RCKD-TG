...
class Constructor(object):
    ...
    def to_seq2seq(self, raw_datasets: DatasetDict, cache_root: str, use_extra_question, use_shuffled_table):
        ...
        train_dataset = TrainDataset(self.args, raw_datasets['train'], cache_root, use_extra_question,
                                     use_shuffled_table)
        ...
        if os.path.exists(cache_path) and args.dataset.use_cache:
            ...
            for expand_id in range(expansion):
                for raw_data in tqdm(self.raw_datasets):
                    ...
                    relevant_cells = extend_data['relevant_cells']

                    table_context = copy.deepcopy(table)
                    # table_shuffled = copy.deepcopy(shuffled_table)

                    '''if extend_data["rationale"] is not None:
                        rationale = extend_data["rationale"].lower()
                    else:
                        pass'''
                    ...
                    '''if extend_data["rationale"] is not None:
                        extend_data.update({"struct_in": linear_table.lower(),
                                            ...
                                            "relevant_cells": relevant_cells.lower(),
