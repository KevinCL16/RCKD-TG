...
class Constructor(object):
    ...
    def to_seq2seq(self, raw_datasets: DatasetDict, cache_root: str, use_extra_question, use_shuffled_table):
        if not len(raw_datasets) == 3:
            raise AssertionError("Train, Dev, Test sections of dataset expected.")
        train_dataset = TrainDataset(self.args, raw_datasets['train'], cache_root, use_extra_question, use_shuffled_table)
...
class TrainDataset(Dataset):
    ...
    def __init__(self, args, raw_datasets, cache_root, use_extra_question, use_shuffled_table):
        ...
        if os.path.exists(cache_path) and args.dataset.use_cache:
            ...
            for expand_id in range(expansion):
                for raw_data in tqdm(self.raw_datasets):
                    # TODO: Commented by Zhiyu: change dict 'extend_data' to add personalized data
                    ...
                    # TODO: Commented by Zhiyu: change content of this dict.update() function to pass personalized data
                    #  format to TokenizedDataset
                    if use_shuffled_table is True and use_extra_question is True:
                        raise AssertionError('only one customized data format can be used. Combination of both not supported yet')

                    if use_shuffled_table is False and use_extra_question is True:
                        shuffled_table = extend_data['shuffled_table']
                        table_shuffled = copy.deepcopy(shuffled_table)
                        linear_shuffled_table = extend_data["meta"] + " " + self.tab_processor.table_linearize_func. \
                            process_table(table_shuffled)

                        question_two = extend_data["extra_question"].lower()
                    ...
                    if use_shuffled_table is True and use_extra_question is False:
                        extend_data.update({
                            "struct_in": linear_table.lower(),
                            "struct_in_shuffled": linear_shuffled_table.lower(),
                            "text_in": question.lower(),
                            "seq_out": gold_result.lower()
                        })
