...
_TRAINING_FILE = "fetaQA-v1_train_st_r-v2_hc.jsonl"
_DEV_FILE = "fetaQA-v1_dev_flan-t5_hc.jsonl"
_TEST_FILE = "fetaQA-v1_test_llm_guided_hc.jsonl"
            ...
            features=datasets.Features(
                {
                    ...
                    # "shuffled_table": {"header": datasets.features.Sequence(datasets.Value("string")),
                                       # "rows": datasets.features.Sequence(datasets.features.Sequence(datasets.Value("string")))},
                    # "rationale": datasets.Value("string"),
                    "relevant_cells": datasets.Value("string"),
    ...
    def _generate_examples(self, filepath):
        ...
        with open(filepath, encoding="utf-8") as f:
            for idx, line in enumerate(f):
                ...
                '''if example["shuffled_table"] is None:
                    example["shuffled_table"] = {"header": None, "rows": None}
                    yield idx, {
                        "id": example["feta_id"],
                        "question": example["question"],
                        "table_id": example["table_source_json"],
                        "table": {"header": example["table_array"][0], "rows": example["table_array"][1:]},
                        "shuffled_table": example["shuffled_table"],
                        "rationale": example["rationale"],
                        "relevant_cells": example["relevant_cells"],
                    ...
                    yield idx, {
                        ...
                        "table_id": example["table_source_json"],
                        "table": {"header": example["table_array"][0], "rows": example["table_array"][1:]},
                        "shuffled_table": {"header": example["shuffled_table"][0],
                                           "rows": example["shuffled_table"][1:]},
                        "rationale": example["rationale"],
                        "relevant_cells": example["relevant_cells"],
                        "meta": example["table_page_title"] + " | " + example["table_section_title"],
                        "answer_text": example["answer"],
